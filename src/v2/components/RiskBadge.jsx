  ...
  return (
    <div className="relative">
      <button
        ...
        className="flex items-center gap-1 rounded-full border border-tracev2-border/70 bg-tracev2-card/70 px-2 py-1 text-tracev2-muted transition-colors duration-150 hover:text-tracev2-text"
      >
        ...
        <>
          ...
          <div className="absolute right-0 top-full z-50 mt-1.5 w-60 rounded-xl border border-tracev2-border bg-tracev2-card p-3 shadow-xl">
            <div className="flex items-center justify-between">
              ...
              <button onClick={onClose} aria-label="Close" className="text-tracev2-subtle hover:text-tracev2-text leading-none">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none">
                  <path d="M6 6l12 12M18 6L6 18" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
                </svg>
              </button>
            </div>
            <p className="mt-1 text-[10px] uppercase tracking-wide text-tracev2-subtle">CTDC indicators</p>
            <ul className="mt-1 space-y-1">
              {indicators.map((ind) => (
                <li key={ind} className="flex gap-1.5 text-[11px] leading-snug text-tracev2-text">
                  <span className="mt-1 h-1 w-1 flex-shrink-0 rounded-full bg-slate-500" />
                  <span>{ind}</span>
                </li>
              ))}
            </ul>
            <p className="mt-2 text-[9px] leading-snug text-tracev2-subtle">
